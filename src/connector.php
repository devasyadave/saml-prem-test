...
if (isset($_SERVER['REQUEST_URI'])) {
    if ($_SERVER['REQUEST_URI'] == '/login') {
        if (DB::get_column('mo_config', 'force_sso') == TRUE) {
            echo '<strong style="margin:34%;">Forced Single Sign On Configured. You will redirected to ' . DB::get_column('mo_config', 'saml_identity_name') . ' IdP</strong>';
...
{
    $status = CD::get_option('mo_saml_registration_status');
    if ($status != 'verified')
        return false;
    else
        return true;
                    ...
                    <table class="mo_saml_settings_table">
                        <tr>
                            ...
                            <td><input class="form-control" type="email" name="email" required
                                       ...
                                       value="<?php echo CD::get_option('mo_saml_admin_email'); ?>"/></td>
                        </tr>
                        <tr>
    ...
    <div class="mo_saml_table_layout">
        ...
        <table border="1"
            ...
            <tr>
                ...
                <td style="width: 55%; padding: 10px;"><?php echo CD::get_option('mo_saml_admin_email'); ?></td>
            </tr>
            <tr>
                <td style="width: 45%; padding: 10px;">Customer ID</td>
                <td style="width: 55%; padding: 10px;"><?php echo CD::get_option('mo_saml_admin_customer_key') ?></td>
            </tr>
        </table>
        <br/> <br/>
        ...
        <form style="display: none;" id="loginform"
            ...
            <input type="email" name="username"
                   value="<?php echo CD::get_option('mo_saml_admin_email'); ?>"/> <input
...
function mo_saml_show_verify_license_page()
{
    ...
    <div class="mo_saml_table_layout"
        ...
        echo '<form style="display:none;" id="loginform" action="' . DB::get_option('mo_saml_host_name') . '/moas/login"
                            target="_blank" method="post">
                            <input type="email" name="username" value="' . CD::get_option('mo_saml_admin_email') . '" />
...
{
    CD::delete_option('mo_saml_admin_customer_key');
    CD::delete_option('mo_saml_admin_api_key');
    CD::delete_option('mo_saml_customer_token');
    CD::delete_option('mo_saml_admin_email');
    CD::delete_option('mo_saml_registration_status');
}
